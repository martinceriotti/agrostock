...
import { useSilos, useSiloAlerts, useSiloReadings, useCreateSilo, useUpdateSilo, type Silo } from '@/lib/hooks/use-silos'
import {
  Database, Thermometer, Droplets, AlertTriangle, CheckCircle2,
  WifiOff, MapPin, Wheat, Plus, Clock, Copy, Check, KeyRound, Loader2, Pencil
} from 'lucide-react'
import { formatDistanceToNow } from 'date-fns'
import { es } from 'date-fns/locale'

// ── Constantes ────────────────────────────────────────────────
const OFFLINE_MS = 5 * 60 * 60 * 1000 // 5 horas
...
const siloSchema     = z.object(baseFields)
const editSiloSchema = z.object({ ...baseFields, empty_date: z.string().optional() })

type SiloFormData     = z.infer<typeof siloSchema>
type EditSiloFormData = z.infer<typeof editSiloSchema>
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="ml-2 rounded p-1 text-gray-400 hover:text-gray-700 transition-colors" title="Copiar">
      {copied ? <Check className="h-4 w-4 text-green-600" /> : <Copy className="h-4 w-4" />}
    </button>
  )
}

// ── Dialog: API key tras crear ────────────────────────────────
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-lg border-2 border-amber-200 bg-amber-50 px-4 py-4 space-y-3">
        ...
        <p className="text-xs text-amber-700">
          Esta clave autentica al dispositivo IoT. También podés verla después desde la tarjeta del silo.
        </p>
      ...
      <div className="text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">
        <p className="font-medium text-gray-700 mb-1">Uso en curl / gateway:</p>
        <code className="text-xs text-gray-600">Authorization: Bearer {silo.api_key.slice(0, 20)}…</code>
      </div>

      <div className="flex justify-end">
        <Button onClick={onClose} className="bg-green-700 hover:bg-green-800">Listo</Button>
      </div>
    </div>
  )
}
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{createdSilo ? 'Silo creado' : 'Nuevo Silo Bolsa'}</DialogTitle>
        </DialogHeader>
        ...
        {createdSilo ? (
          ...
          <SiloForm
            ...
            isPending={createSilo.isPending}
            submitLabel="Crear silo"
          />
        )}
      </DialogContent>
    </Dialog>
...
// ── Dialog: Editar silo ───────────────────────────────────────
function EditSiloDialog({ silo, open, onClose }: { silo: Silo; open: boolean; onClose: () => void }) {
  const updateSilo = useUpdateSilo()

  const form = useForm<EditSiloFormData>({
    resolver: zodResolver(editSiloSchema),
    defaultValues: {
      name:          silo.name,
      field_name:    silo.field_name  ?? '',
      crop:          silo.crop        ?? '',
      capacity_tons: silo.capacity_tons ?? undefined,
      fill_date:     silo.fill_date   ?? '',
      empty_date:    silo.empty_date  ?? '',
      status:        silo.status,
      notes:         silo.notes       ?? '',
    },
  })

  function onSubmit(data: EditSiloFormData) {
    updateSilo.mutate({
      id:            silo.id,
      name:          data.name,
      field_name:    data.field_name  || null,
      ...
      capacity_tons: isNaN(data.capacity_tons as number) ? null : (data.capacity_tons ?? null),
      fill_date:     data.fill_date   || null,
      empty_date:    data.empty_date  || null,
      status:        data.status,
      notes:         data.notes       || null,
    }, {
      onSuccess: onClose,
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Editar silo — {silo.name}</DialogTitle>
        </DialogHeader>
        <SiloForm
          form={form}
          onSubmit={onSubmit}
          onCancel={onClose}
          isPending={updateSilo.isPending}
...
function SiloForm({
  form, onSubmit, onCancel, isPending, submitLabel, showEmptyDate = false,
}: {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v: string) => form.setValue('status', v)}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {showEmptyDate && (
          <div className="space-y-1.5">
            <Label>Fecha de vaciado</Label>
            <Input type="date" {...form.register('empty_date')} />
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end gap-2 pt-1">
        ...
        <Button type="submit" disabled={isPending} className="bg-green-700 hover:bg-green-800 gap-2">
          {isPending ? <><Loader2 className="h-4 w-4 animate-spin" />Guardando…</> : submitLabel}
        </Button>
      </div>
    </form>
  )
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button size="sm" className="gap-2 bg-green-700 hover:bg-green-800" onClick={() => setShowCreate(true)}>
...
function SiloCard({ silo }: { silo: Silo }) {
  ...
  const isOffline = silo.status === 'active' && (
    !latest || Date.now() - new Date(latest.recorded_at).getTime() > OFFLINE_MS
  )
  ...
  return (
    <>
        ...
        <CardContent className="pt-4 pb-3">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {isOffline ? (
                <Badge className="text-xs border bg-gray-100 text-gray-500 border-gray-300 flex items-center gap-1">
                  <WifiOff className="h-3 w-3" /> Sin conexión
                </Badge>
              ) : (
                <Badge className={`text-xs border ${STATUS_COLORS[silo.status]}`}>
          ...
          {latest ? (
            ...
            <p className="text-xs text-gray-400 -mt-1 mb-2">Sin lecturas registradas</p>
          )}
      ...
      <EditSiloDialog silo={silo} open={showEdit} onClose={() => setShowEdit(false)} />
    </>
  )
}
